  ...
  createScatter(): void {
    ...
    this.visualizationData.clusterData[0].forEach((cluster, index) => {
      cluster.points.forEach((point) => {
        ...
        this.options.set(
          this.visualizationDataService.getOption(point, this.selectedFeature),
          true
        );
      });
    });
  ...
  private drawPlot(): void {
    ...
    this.dataPoints = this.gPlot
      ...
      .attr(
        "cx",
        (d: any) =>
          this.x(this.visualizationDataService.getX(d, this.selectedFeature)) +
          this.margin
      )
      .attr("cy", (d: any) =>
        this.y(this.visualizationDataService.getY(d, this.selectedFeature))
      )
      .attr("r", 7)
      .style("opacity", 0.5)
      ...
      .on("mouseover", function (event, d) {
        const vizBox = document
          .querySelector(
            "#scatterClustersSvgPlaceholder kypo-clustering-visualization"
          )
  ...
  private updateChart(event: any) {
    ...
    this.gPlot
      ...
      .attr(
        "cx",
        (d: any) =>
          newX(this.visualizationDataService.getX(d, this.selectedFeature)) +
          this.margin
      )
      .attr("cy", (d: any) =>
        newY(this.visualizationDataService.getY(d, this.selectedFeature))
      );
